...
module PeekEmbeddings
  class List < ApplicationComponent
    ...
    def template
      turbo_frame_tag(:peek_list_embeddings) do
        h3(class: "mt-4 text-2xl leading-7 text-skin-muted") { t("peek_embeddings.list.description", size: @embeddings.size) }
      ...
      li(class: "flex flex-wrap justify-between gap-x-6 gap-y-4 py-5 sm:flex-nowrap place-items-start", id: embedding.id) do
        div(class: "w-full") do
          if display_section?(:documents)
            div(class: "mt-3") do
              h3(class: "text-base font-semibold leading-6 text-skin-muted") { t("embedding_results.document", id: embedding.id) }
              div(class: "mt-1 text-base leading-6 text-gray-500 border w-full p-4 rounded-md overflow-y-scroll h-44") { simple_format(embedding.document) }
            end
          end
          ...
          if display_section?(:embeddings)
            div(class: "mt-3") do
              ...
              div(class: "mt-3 text-base leading-6 text-gray-500 border w-full p-4 rounded-md overflow-y-scroll h-44") { embedding.embedding&.to_s }
            end
          end
