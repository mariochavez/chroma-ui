...
module QueryEmbeddings
  class Form < ApplicationComponent
    ...
    LIMIT = (1..10).to_a
    ADDITIONAL_QUERY = ["documents, metadatas, distances", "embeddings, documents, metadatas, distances"]
    ...
    def template
      turbo_frame_tag(:query_embeddings) do
        h3(class: "text-base leading-6 text-skin-muted") { t("query_embeddings.form.description") }
        form_with model: @query, url: query_embeddings_path, class: "mt-4 grid gap-6 grid-cols-1 md:grid-cols-4", data: {panel_target: :form} do |form|
          form.hidden_field :collection_id
          hidden_field_tag :collection_name, @collection_name

          div(class: "flex md:col-span-2 flex-col") do
            div do
              form.label(:limit, class: "label")
              div(class: "mt-2") do
                form.select(:limit, limit_values, {}, class: "select w-full md:w-3/4")
              end
            end
            ...
            div(class: "mt-4") do
              ...
              div(class: "mt-2") do
                form.select(:additional, additional_values, {}, class: "select w-full md:w-3/4")
              end
            end
          end

          div(class: "md:col-span-2") do
            attribute_error = attribute_error(:embeddings)
            form.label(:embeddings, class: "label")
            div(class: "mt-2") do
              form.text_area(:embeddings, class: token_list("input w-full h-52 resize-none", "input-error": attribute_error.present?), placeholder: t("query_embeddings.form.placeholders.embeddings"))
              p(class: "help") { t("query_embeddings.form.help.embeddings") }
